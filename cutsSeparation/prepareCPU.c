...
Cut_gpu* initial_runCPU(Cut_gpu *h_cut, int precision, double timeconst)
{
    ...
    Cut_gpu* out_h_cut;
    int nRuns = h_cut->numberConstrains;
    int i, numberC = 0 ;
    //int nC_initial = 0, cont_ini;
    //numberC = h_cut->numberConstrains;
...
void saveSoluctionFrac(double *xTemp, int numberVariables, char *nameFileOut, LinearProgramPtr lp, int round)
{
    FILE *fout;
    char tempName[255];
    char VariablesName[255];
    int i;
    char *teste;
    teste = strstr(nameFileOut,"/") + 1;
    sprintf(tempName, "../saidaTemp/Sol_temp_%s_round_%d.txt", teste,round);

    fout = fopen(tempName,"w");
    for(i = 0; i <numberVariables; i++)
    {
        lp_col_name(lp,i,VariablesName);
        fprintf(fout,"%s = %f\n", VariablesName,xTemp[i]);
    }
    fclose(fout);
    sprintf(tempName,"../lpTemp/LP_temp_%s_round_%d.lp", teste,round);
    lp_write_lp(lp,tempName);
}
...
Cut_gpu *onlyVariablesActive(Cut_gpu *h_cut, LinearProgramPtr lp)
{
    ...
    int *varActived = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int i, j, el = 0;
    ...
    for(i=0; i<h_cut->numberConstrains; i++)
    {
        if(actived[i]==1)
        {
            for(j=h_cut->ElementsConstraints[i]; j<h_cut->ElementsConstraints[i+1]; j++)
            {
                ...
                if(xTemp[el]!=0)
                {
                    //printf("%d - %d ",h_cut->Coefficients[j],el);
                    new_Contraints->Coefficients[cont] = h_cut->Coefficients[j];
                    new_Contraints->Elements[cont] = el;
                    cont++;
                }
            }
            //printf("%d -- %d x %d -- %d ",new_Contraints->ElementsConstraints[cont_constraints], cont, h_cut->ElementsConstraints[i], h_cut->ElementsConstraints[i+1]);
            //printf("<= %d", h_cut->rightSide[i]);
            //printf("\n");
            ...
            new_Contraints->ElementsConstraints[cont_constraints + 1] = cont;
            cont_constraints++;
//            printf("NEWS!\n");
//            show_contraints(h_cut,i);
//            show_contraints(new_Contraints,cont_constraints-1);
////            //getchar();
        }
    }

    int *isRepeat = (int*)malloc(sizeof(int)*new_Contraints->numberConstrains);
    for(i=0; i<new_Contraints->numberConstrains; i++)
    {
        isRepeat[i] = 0;
    }
    for(i=0; i<new_Contraints->numberConstrains; i++)
    {
        for(j = i+1; j<new_Contraints->numberConstrains; j++)
        {
            isRepeat[j] += verifyRepeatCuts(new_Contraints,j,i);
        }
    }
    cont = new_Contraints->cont;
    int nConstraintsN = new_Contraints->numberConstrains;
    for(i=0; i<new_Contraints->numberConstrains; i++)
    {
        if(isRepeat[i]>0)
        {
            cont -= (new_Contraints->ElementsConstraints[i+1] - new_Contraints->ElementsConstraints[i]);
            nConstraintsN --;
        }
    }
    Cut_gpu *new_Contraints_no_repeat = AllocationStructCut(cont,nConstraintsN,new_Contraints->numberVariables);
    for(i=0; i<new_Contraints_no_repeat->numberVariables; i++)
    {
        new_Contraints_no_repeat->xAsterisc[i] = new_Contraints->xAsterisc[i];
    }
    new_Contraints_no_repeat->ElementsConstraints[0] = 0;
    cont = 0;
    cont_constraints = 0;
    for(i=0; i<new_Contraints->numberConstrains; i++)
    {
        if(isRepeat[i]==0)
        {
            for(j=new_Contraints->ElementsConstraints[i]; j<new_Contraints->ElementsConstraints[i+1]; j++)
            {


                //printf("%d - %d ",h_cut->Coefficients[j],el);
                new_Contraints_no_repeat->Coefficients[cont] = new_Contraints->Coefficients[j];
                new_Contraints_no_repeat->Elements[cont] = new_Contraints->Elements[j];
                cont++;
            }
            //printf("<= %d", h_cut->rightSide[i]);
            //printf("\n");
            new_Contraints_no_repeat->rightSide[cont_constraints] = new_Contraints->rightSide[i];
            new_Contraints_no_repeat->typeConstraints[cont_constraints] = new_Contraints->typeConstraints[i];
            cont_constraints++;
            new_Contraints_no_repeat->ElementsConstraints[cont_constraints] = cont;

        }
    }
//    for(i=0;i<new_Contraints_no_repeat->numberConstrains;i++){
//        show_contraints(new_Contraints_no_repeat,i);
//    }
//    getchar();
    //free(xTemp);
    free(isRepeat);
    free(new_Contraints);
    free(actived);
    return new_Contraints_no_repeat;
}
...
int *returnVectorParametersConstraints(Cut_gpu *h_cut, int Constraints, int sizeGroup, float pCom, float pCoef, float pSignal, float pFrac, int precision){
    int *groupConstraints = (int*)malloc(sizeof(int)*sizeGroup);
    int *countEquals = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int *countCoef = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int *countSignalDif = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int *countFrac = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    float *total = (float*)malloc(sizeof(float)*h_cut->numberConstrains);
    int *idc = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int *FractinalSolution = (int*)malloc(sizeof(int)*h_cut->numberVariables);
    int i, j, k,z;

    for(i=0;i<h_cut->numberVariables;i++){
        if( h_cut->xAsterisc[i]%precision != 0){
            if(h_cut->xAsterisc[i]<0){
                k = h_cut->xAsterisc[i]/precision;
                 //printf("%d %d %d \n", k, h_cut->xAsterisc[i], precision);
                FractinalSolution[i]= -1*(k)*precision - h_cut->xAsterisc[i];    
            }else{
                k = h_cut->xAsterisc[i]/precision;
                //printf("%d %d %d \n", k, h_cut->xAsterisc[i], precision);
                FractinalSolution[i]= (k+1)*precision - h_cut->xAsterisc[i];
            }
    ...
    for (z = h_cut->ElementsConstraints[Constraints]; z< h_cut->ElementsConstraints[Constraints+1]; z++)
    {
        for (i = 0; i<h_cut->numberConstrains; i++)
        {
            if(i!=Constraints)
            {
                for(j = h_cut->ElementsConstraints[i]; j<h_cut->ElementsConstraints[i+1]; j++)
                {
                    if(h_cut->Elements[z] == h_cut->Elements[j])
                    {
                        countEquals[i]++;
                        countCoef[i] += h_cut->Coefficients[j];
                        if(h_cut->Coefficients[j]*h_cut->Coefficients[z]<0){
                            countSignalDif[i]++;
                        }
                        int el = h_cut->Elements[z];
    ...
    for (i=0; i<h_cut->numberConstrains; i++)
    {
        total[i] = pCom*(float)countEquals[i] + pCoef*(float)countCoef[i] +pSignal*(float)countSignalDif[i] + pFrac*(float)countFrac[i];
    }
    free(countCoef);
    free(countEquals);
    free(countSignalDif);
    free(countFrac);
    quicksortFloat(total, idc, 0,h_cut->numberConstrains);
    j = 0;
    int posMaior = 0;
    if (posMaior + sizeGroup < h_cut->numberConstrains-1){
        for(i = posMaior; i<posMaior + sizeGroup; i++)
        {
            groupConstraints[j] = idc[i];
            j++;
        }
    }else{
        for(i = h_cut->numberConstrains - (sizeGroup+1); i<h_cut->numberConstrains - 1;i++){
            groupConstraints[j] = idc[i];
            j++;
        }
    }
...
int *returnVectorMaxComConstraints(Cut_gpu *h_cut, int Constraint, int sizeGroup, int posMaior)
{
    int *groupConstraints = (int*)malloc(sizeof(int)*sizeGroup);
    int *countEquals = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int *posEquals = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int i, j, k,z;
    for (i=0; i<h_cut->numberConstrains; i++)
    {
        posEquals[i] = i;
    ...
    for (z = h_cut->ElementsConstraints[Constraint]; z< h_cut->ElementsConstraints[Constraint+1]; z++)
    {
        for (i = 0; i<h_cut->numberConstrains; i++)
        {
            if(i!=Constraint)
    ...
    quicksortDesc(countEquals,posEquals,0,h_cut->numberConstrains);
    //show_contraints(h_cut,Constraint);
    j = 0;
    if (posMaior + sizeGroup < h_cut->numberConstrains-1){
        for(i = posMaior; i<posMaior + sizeGroup; i++)
        {
            groupConstraints[j] = posEquals[i];
            j++;
        }
    }else{
        for(i = h_cut->numberConstrains - (sizeGroup+1); i<h_cut->numberConstrains - 1;i++){
...
int *returnVectorMaxCoefConstraints(Cut_gpu *h_cut, int sizeGroup, int posMaior){
    int *groupConstraints = (int*) malloc (sizeof(int)*sizeGroup);
    int *countCoef = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int *posCoef = (int*)malloc(sizeof(int)*h_cut->numberConstrains);
    int i,j;
    for(i=0;i<h_cut->numberConstrains;i++){
        countCoef[i] = 0;
        posCoef[i] = 0;
    ...
    quicksortDesc(countCoef,posCoef,0,h_cut->numberConstrains);
    j = 0;
    if (posMaior + sizeGroup < h_cut->numberConstrains){
        for(i = posMaior; i<posMaior + sizeGroup; i++)
        {
            groupConstraints[j] = posCoef[i];
            j++;
        }
    }else{
        for(i = h_cut->numberConstrains - sizeGroup; i < h_cut->numberConstrains;i++){
            groupConstraints[j] = posCoef[i];
            j++;
        }
    }
...
Cut_gpu* new_second_phase_runCPU(Cut_gpu *h_cut, int numberMaxConst, int nRuns, int maxDenominator, int precision, double timeLeft)
{
    Cut_gpu *out_cut_gpu;
    int *constraints;
    int *nElementsConstraints;
    char *typeConstraints = (char*)malloc(sizeof(char)*h_cut->numberConstrains); //0 or 1 for more two variables actives
    int i,j,  el, aux;
    for(i = 0; i<h_cut->numberConstrains + 1; i++)
    {
        typeConstraints[i] = 0;
        aux = 0;
        for(j = h_cut->ElementsConstraints[i]; j< h_cut->ElementsConstraints[i+1]; j++)
        {
            el = h_cut->Elements[j];
            if((h_cut->Coefficients[j]!=0)&&(h_cut->xAsterisc[el]!=0))
            {
                aux++;
            }
            if(aux==2)
...
Cut_gpu* second_phase_runCPU(Cut_gpu *h_cut, int sizeGroup, int nRuns, int maxDenominator, int precision, int posMaior, double timeLeft,  float pCom, float pCoef, float pSignal, float pFrac)
{
    ...
    solutionGpu *h_solution_r2 = allocationStructSolution2(h_cut,sizeGroup,nRuns);
    _time = ( (double) timeLeft - (omp_get_wtime()-startT) );
    if(_time<1)
    {
        free(h_solution_r2);
    ...
    int *setConstraint = (int*)malloc(sizeof(int)*sizeGroup*nRuns);
    int *auxSet = (int*)malloc(sizeof(int)*sizeGroup);
    ...
    for(i = 0;i<nRuns;i++){
        if (op==1){
            int p = i % h_cut->numberConstrains;
            auxSet = returnVectorMaxComConstraints(h_cut,i, sizeGroup,posMaior);
        }
        if(op==2){
            int p = i % h_cut->numberConstrains;
            
            auxSet = returnVectorParametersConstraints(h_cut,p,sizeGroup,pCom,pCoef,pSignal,pFrac, precision);
        }
        for(j = 0 ; j< sizeGroup; j++){
            setConstraint[j + i*sizeGroup] = auxSet[j];
        }
        free(auxSet);
    }
    shuffle_Set(setConstraint, sizeGroup, sizeGroup*nRuns);
//    for (i = 0; i<nRuns;i++){
//        for(j=0;j<sizeGroup;j++){
//            printf(" %d ", setConstraint[j + i*sizeGroup]);
//        }
//        printf("\n");
//    }
    ...
    runCPUR2(h_cut, h_solution_r2, sizeGroup,setConstraint,precision,maxDenominator,nRuns,_time);
    int cont=0;
    _time = ( (double) timeLeft - (omp_get_wtime()-startT) );
    if(_time<1)
    {
    ...
    for(i=0; i<nRuns; i++)
    {
        if(h_solution_r2->SConst[0 + i*sizeGroup]!=-1)
        {
            cont++;
        }
    }
    if(cont>0)
    {
        ...
        int nT = nRuns;
        int nB = 1;
        out_cut_gpu = createCutsStrongPhaseTwo(h_cut,h_solution_r2,sizeGroup,cont,precision,nRuns,nT,nB);
    }
    else
    {
